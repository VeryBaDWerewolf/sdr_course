...
static struct iio_context *ctx1 = NULL;
...
static void shutdown()
{
	...
	if (ctx1) {iio_context_destroy(ctx1); }

}

void set_dev(struct iio_device **phy_rx, struct iio_device **phy_tx, struct iio_device **tx,
             struct iio_device **rx) {
  
  *tx = iio_context_find_device(ctx, "cf-ad9361-dds-core-lpc");
  *rx = iio_context_find_device(ctx1, "cf-ad9361-lpc");
  
  *phy_rx = iio_context_find_device(ctx1, "ad9361-phy");
  *phy_tx = iio_context_find_device(ctx, "ad9361-phy");
}
...
int main() {
  ...
  ctx1 = iio_create_context(NULL, "ip:192.168.2.1");
  
  printf("Hello\n");
 
  struct iio_device *phy_tx;
  struct iio_device *phy_rx;
  ...
  set_dev(&phy_rx, &phy_tx, &tx_dev, &rx_dev);
  ...
  set_param(tx_chn, tx_lo_chn, phy_tx, txcfg, TX);
  set_param(rx_chn, rx_lo_chn, phy_rx, rxcfg, RX);
